...
var googleMapsController = {
    ...
    heightMapDiv : null,        // dort wird das Canvas als Child eingefügt bzw. entfernt (wird von außen gesetzt)

    // Setter für Html-div
    setHeightMapDiv : function(div) {
        this.heightMapDiv = div;
    },
    ...
    showPathAndHeightMap : function(trackId) {
        GoogleMapsLoader.load(function (google) {
		    ...
		    fetchTrack(trackId).then(jsonData => {
                ...
                // bei 'this' keinen Verweis auf die Funktion createHeightMap() hat! Daher muss im callback
                // die Funktion manuell auf dem Objekt aufgerufen werden
    ...
    createHeightMap : function(heightCoordinates) {
	    ...
	    if(this.heightMapCanvas) {
		    this.heightMapDiv.removeChild(heightMapCanvas);
	    }
	    ...
	    this.heightMapDiv.appendChild(this.heightMapCanvas);
